...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const media = await getRuntimeDb().prepare(`SELECT m.*,c.status content_status,c.visibility content_visibility,c.scheduled_at,c.deleted_at content_deleted_at,c.published_media_id,c.published_media_state,c.is_demo,(SELECT value FROM platform_settings WHERE key='demo_visibility') demo_visibility,EXISTS(SELECT 1 FROM demo_data_items di JOIN demo_data_batches dbat ON dbat.id=di.batch_id WHERE di.content_id=c.id AND di.media_asset_id=m.id AND di.cleanup_status='active' AND dbat.status='active') demo_owned,(m.id=(SELECT legacy.id FROM media_assets legacy WHERE legacy.content_id=c.id AND legacy.approval_status='approved' AND legacy.media_status='active' ORDER BY legacy.created_at DESC,legacy.id DESC LIMIT 1)) legacy_current,EXISTS(SELECT 1 FROM site_content_entries e WHERE e.published_json LIKE '%"mediaId":'||m.id||'%') cms_published FROM media_assets m LEFT JOIN content_records c ON c.id=m.content_id WHERE m.id=? AND m.media_status='active'`).bind(id).first<{ r2_key: string; mime_type: string; owner_user_id: number; approval_status: string;content_id:number|null;content_status:string|null;content_visibility:string|null;scheduled_at:string|null;content_deleted_at:string|null;published_media_id:number|null;published_media_state:string|null;legacy_current:number;cms_published:number;is_demo:number|null;demo_visibility:string|null;demo_owned:number }>();
    if (!media) throw new PlatformError(404, "Imaginea nu există.");
    const scheduledPublic=media.content_status==="scheduled"&&media.scheduled_at&&new Date(media.scheduled_at).getTime()<=Date.now();const publishedSelection=media.published_media_state==="selected"?media.published_media_id===id:media.published_media_state==="legacy"&&Boolean(media.legacy_current);const demoPublic=media.is_demo!==1||(media.demo_visibility==="public"&&Boolean(media.demo_owned));const publiclyAvailable=media.approval_status==="approved"&&(Boolean(media.cms_published)||(Boolean(media.content_id)&&publishedSelection&&demoPublic&&media.content_visibility==="public"&&!media.content_deleted_at&&(media.content_status==="published"||scheduledPublic)));
