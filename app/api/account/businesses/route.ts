...
export async function POST(request: Request) {
  try {
    ...
    const content = await createContent(account, {
      type: "business",
      title: cleanText(body.name, 240, true),
      excerpt: cleanText(body.description, 6000, true),
      locality: cleanText(body.locality, 120, true),
      categoryId: Number(body.categoryId) || null,
      sourceUrl: cleanText(body.evidenceUrl, 800),
      details: { address: body.address, phone: body.phone, website: body.website, description: body.description },
    });
    const explanation = cleanText(body.explanation || body.evidence, 3000, true);
    ...
    await db.batch([
      ...
      db.prepare("UPDATE content_records SET status='pending_review',moderation_state='pending_review',submitted_at=CURRENT_TIMESTAMP WHERE id=?").bind(content.id),
