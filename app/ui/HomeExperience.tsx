...
function EventCard({ event, detailsLabel, emphasized }: { event: PublicEvent; detailsLabel: string; emphasized: boolean }) {
  return <article className={`home-event-card ${emphasized ? "is-leading" : ""}`}><div className="home-event-media"><img src={event.image} alt={event.imageAlt || ""} width="720" height="480" loading="lazy" sizes="(max-width: 700px) calc(100vw - 28px), (max-width: 1100px) 50vw, 420px" />{event.isDemo && <span className="demo-badge">Conținut demonstrativ</span>}</div><div className="home-event-body">
    ...
    {detailsLabel && <Link className="home-card-link" href={`/evenimente/${event.id}`}>{detailsLabel}<ArrowRight size={16} aria-hidden="true" /></Link>}
  </div></article>;
}
...
function BusinessCard({ business, profileLabel }: { business: PublicBusiness; profileLabel: string }) {
  return <article className="home-service-card">{business.image ? <img src={business.image} alt={business.imageAlt || ""} width="720" height="480" loading="lazy" sizes="(max-width: 700px) calc(100vw - 28px), 360px" /> : <div className="home-service-mark" aria-hidden="true">{business.name.charAt(0)}</div>}<div>{business.promoted && <span className="promoted">Promovat</span>}{business.category && <p className="home-eyebrow">{business.category}</p>}<h3>{business.name}</h3>{business.locality && <p className="home-location"><MapPin size={16} aria-hidden="true" />{business.locality}</p>}{business.isDemo && <span className="demo-line">Exemplu demonstrativ</span>}{profileLabel && <Link className="home-card-link" href={`/afaceri-si-servicii/${business.id}`}>{profileLabel}<ArrowRight size={17} aria-hidden="true" /></Link>}</div></article>;
}
...
function OffersSection({ content, offers }: { content: HomeCopy; offers: PublicCatalog["offers"] }) {
  ...
  return <section className="home-module home-offers"><div className="container">{heading && <SectionHead {...heading} />}<div className={`home-offer-grid count-${shown.length}`} data-count={shown.length}>{shown.map(offer => <article className="home-offer-card" key={offer.id}>{offer.isDemo && <p className="demo-line">Exemplu demonstrativ</p>}<h3>{offer.title}</h3>{offer.business && <p>{offer.business}</p>}<div className="home-price">{offer.old && <del>{offer.old}</del>}<strong>{offer.price}</strong></div>{value(content, "offersUntilLabel") && offer.until && <p className="home-location"><Clock3 size={16} aria-hidden="true" />{value(content, "offersUntilLabel")} {offer.until}</p>}{value(content, "offersConditionsLabel") && <Link className="home-card-link" href={`/oferte-locale/${offer.id}`}>{value(content, "offersConditionsLabel")}<ArrowRight size={16} aria-hidden="true" /></Link>}</article>)}</div></div></section>;
}
...
function RestaurantsSection({ content, restaurants, shown, filters, activeFilter, onFilter }: { content: HomeCopy; restaurants: PublicRestaurant[]; shown: PublicRestaurant[]; filters: RepeatableItem[]; activeFilter: string; onFilter: (id: string) => void }) {
  ...
  return <section className="home-module home-restaurants"><div className="container">{heading && <SectionHead {...heading} />}{filters.length > 0 && <div className="home-filters" role="group" aria-label={value(content, "restaurantsFiltersLabel") || undefined}>{filters.map((filter, index) => { const id = filterId(filter, `food-${index}`); return <button type="button" aria-pressed={activeFilter === id} onClick={() => onFilter(id)} className={activeFilter === id ? "active" : ""} key={filter.id || `${filter.value}-${index}`}>{filter.value}</button>; })}</div>}<div className="home-food-list">{shown.slice(0, 5).map(restaurant => <article className="home-food-row" key={restaurant.id}><div>{restaurant.type && <p className="home-eyebrow">{restaurant.type}{restaurant.isDemo ? " · Exemplu demonstrativ" : ""}</p>}<h3>{restaurant.name}</h3>{restaurant.dish && <p>{restaurant.dish}</p>}</div>{restaurant.price && <strong>{restaurant.price}</strong>}{value(content, "restaurantsDetailsLabel") && <Link href={`/unde-mancam/${restaurant.id}`} aria-label={`${value(content, "restaurantsDetailsLabel")} — ${restaurant.name}`}><ArrowRight aria-hidden="true" /></Link>}</article>)}</div></div></section>;
}
...
function JobsSection({ content, jobs }: { content: HomeCopy; jobs: PublicCatalog["jobs"] }) {
  ...
  return <section className="home-module home-jobs"><div className="container">{heading && <SectionHead {...heading} />}<div className="home-job-list">{jobs.slice(0, 4).map(job => <article className="home-job-row" key={job.id}><div>{job.isDemo && <span className="demo-badge">Exemplu demonstrativ</span>}<h3>{job.title}</h3><p>{[job.company, job.locality].filter(Boolean).join(" · ")}</p></div>{value(content, "jobsScheduleLabel") && <div><small>{value(content, "jobsScheduleLabel")}</small><strong>{[job.type, job.schedule].filter(Boolean).join(" · ")}</strong></div>}{value(content, "jobsSalaryLabel") && <div><small>{value(content, "jobsSalaryLabel")}</small><strong>{job.salary}</strong></div>}{value(content, "jobsDetailsLabel") && <Link href={`/locuri-de-munca/${job.id}`}>{value(content, "jobsDetailsLabel")}<ArrowRight size={16} aria-hidden="true" /></Link>}</article>)}</div></div></section>;
}
