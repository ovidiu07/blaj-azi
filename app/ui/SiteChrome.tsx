      ...
      <header className="site-header">
        <div className="nav-wrap">
          ...
          <div className="header-actions">
            ...
            <button ref={menuTrigger} className="icon-button menu-button" aria-label={label(content, "menuLabel")} aria-expanded={open} aria-controls="site-mobile-menu" onClick={() => setOpen(true)}><Menu size={22} aria-hidden="true" /></button>
          </div>
        </div>
      </header>
      {open && <div id="site-mobile-menu" ref={menuDialog} className="overlay-panel" role="dialog" aria-modal="true" aria-labelledby="mobile-menu-title">
        <div className="overlay-top"><Logo homeLabel={label(content, "homeLabel")} /><h2 id="mobile-menu-title" className="sr-only">Meniu Blaj Azi</h2><button className="icon-button" aria-label="Închide meniul" onClick={() => setOpen(false)}><X aria-hidden="true" /></button></div>
        <nav className="mobile-nav" aria-label="Navigație mobilă">
          <section className="mobile-nav-section"><h2>Explorează</h2>{mainNav.map(item => <Link key={item.href} href={item.href} aria-current={pathname.startsWith(item.href)?"page":undefined} onClick={() => setOpen(false)}>{item.label}</Link>)}{supplemental.map(item => <Link href={item.href} key={item.href} aria-current={pathname.startsWith(item.href)?"page":undefined} onClick={() => setOpen(false)}>{item.label}</Link>)}</section>
          <section className="mobile-nav-section"><h2>Pentru comunitate</h2><Link className="mobile-create-link" href="/adauga" aria-current={pathname==="/adauga"?"page":undefined} onClick={() => setOpen(false)}><Plus aria-hidden="true"/>{label(content, "addLabel")}</Link></section>
          <section className="mobile-nav-section"><h2>Cont</h2>{account ? <><Link href="/cont" aria-current={pathname==="/cont"?"page":undefined} onClick={() => setOpen(false)}>{label(content, "accountLabel")}</Link><Link href="/cont/continut" aria-current={pathname.startsWith("/cont/continut")?"page":undefined} onClick={() => setOpen(false)}>Conținutul meu</Link>{["business_owner","admin","platform_owner"].includes(account.globalRole) && <Link href="/cont/afaceri" aria-current={pathname.startsWith("/cont/afaceri")?"page":undefined} onClick={() => setOpen(false)}>Afacerea mea</Link>}<Link href="/deconectare" onClick={() => setOpen(false)}>Deconectare</Link></> : <><Link href="/conectare?return_to=%2Fcont" aria-current={pathname==="/conectare"?"page":undefined} onClick={() => setOpen(false)}>{label(content, "loginLabel")}</Link><Link href="/inregistrare?return_to=%2Fcont" aria-current={pathname==="/inregistrare"?"page":undefined} onClick={() => setOpen(false)}>{label(content, "registerLabel")}</Link></>}</section>
          {account&&["admin","platform_owner"].includes(account.globalRole)&&<section className="mobile-nav-section"><h2>Administrare</h2><Link href="/admin" aria-current={pathname.startsWith("/admin")?"page":undefined} onClick={() => setOpen(false)}>{label(content, "adminLabel")}</Link></section>}
        </nav>
      </div>}
      {search && <div id="site-search-dialog" ref={searchDialog} className="search-overlay" role="dialog" aria-modal="true" aria-label="Căutare în site">
        <button className="icon-button search-close" aria-label="Închide căutarea" onClick={() => setSearch(false)}><X aria-hidden="true" /></button>
